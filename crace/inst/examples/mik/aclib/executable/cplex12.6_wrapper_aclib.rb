...
load  "./aclib/executable/cplex12.6_wrapper.rb"
	...
	File.open(tmp_file){|file|
		while line = file.gets
			...
			if line =~ /Solution time =\s*(#{float_regexp}) sec\./
				internal_measured_runtime = $1
				raise "CPLEX reports negative time. I thought this bug was fixed in version 12.6." if internal_measured_runtime.to_f < 0
			end
...
#				end
			...
			if line =~ /runsolver_max_cpu_time_exceeded/
				puts "runsolver_max_cpu_time_exceeded"
				solved = "TIMEOUT"
			end
			if line =~ /runsolver_max_memory_limit_exceeded/
				puts "runsolver_max_memory_limit_exceeded"
				solved = "TIMEOUT"
			end
			if line =~ /Real time \(s\): (#{float_regexp})$/
				walltime = $1.to_f
			end
			if line =~ /total CPU user time \(s\): (#{float_regexp})$/
				runtime = $1.to_f
			end
		end
	}
	...
	begin
		# File.delete(tmp_file) unless solved == "CRASHED"
		File.delete(tmp_file)
