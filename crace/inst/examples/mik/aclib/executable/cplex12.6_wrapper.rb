...
# Deal with inputs.
if ARGV.length < 13
    puts "cplex12.6_wrapper.rb is a wrapper for CPLEX"
    puts "Usage: ruby cplex12.6_wrapper.rb --memoryLimit <memLimit in MB> --runsolverLocation <directory with runsolver> --allowedMIPgap <allowed MIP gap, e.g. 0.0001> --objFileLocation <location of objective value file, or 'none'> <instance_relname> <instance_specifics (string in \"quotes\"> <cutoff_time> <cutoff_length> <seed> <params to be passed on as -param value pairs>."
    exit -1
end
...
ARGV[0..7].each_index { |i|
		...
		elsif (ARGV[i] == "--objFileLocation")
			$objFileLocation = ARGV[i+1]
                end
        end
}
...
cmd = "ruby #{File.dirname(__FILE__)}/run_cplex12.6.rb #{$instance} #{$runtimeLimit} #{memoryLimit}"
